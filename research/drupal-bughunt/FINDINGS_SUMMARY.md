...
## CONFIRMED pre-auth findings (34, source-verified)
...
| 33 | **apex_ai** | CRITICAL | pre-auth **SQL injection** — anon `/api/apex-ai/search` request `collection` → `vectorSearch` → `db->query("... FROM \"vdb_{$collection}\"")` raw concat (found by CodeQL taint) | **current** (^11.2) |
| 34 | **azure_acs** | MED | pre-auth **reflected/session XSS** — anon `/acserror` puts `$_POST[ErrorDetails].errorMessage` into `drupal_set_message` (D7 renders HTML unescaped); forced-POST delivery, no CSRF (found by CodeQL) | D7 |
